...
tesseract::Dawg *LoadSquishedDawg(const UNICHARSET &unicharset,
  ...
  FILE *dawg_file;//Added on 6th April - Vasu
  fopen_s(&dawg_file, filename, "rb"); //Added on 6th April - Vasu

  //FILE *dawg_file = fopen(filename, "rb"); //Commented on 6th April - Vasu
  if (dawg_file == NULL) {
...
int WriteDawgAsWordlist(const UNICHARSET &unicharset,
	...
	//FILE *out = fopen(outfile_name, "wb"); //Commented on 6th April - Vasu 
  FILE *out;	//Added on 6th April - Vasu
  fopen_s(&out, outfile_name, "wb"); //Added on 6th April - Vasu
