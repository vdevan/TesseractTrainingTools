...
inline bool LoadDataFromFile(const STRING& filename,
	...
	//FILE* fp = fopen(filename.string(), "rb"); //Commented on 5th April - Vasu
	FILE* fp; //Added on 5th April - Vasu
	fopen_s(&fp, filename.string(), "rb"); //Added on 5th April - Vasu
...
inline bool SaveDataToFile(const GenericVector<char>& data,
	...
	//FILE* fp = fopen(filename.string(), "wb"); //Commented on 5th April - Vasu
  if (fp == NULL) return false;
  bool result =
