...
class TESS_API STRING
{
  public:
    ...
    inline char* strdup() const {
     inT32 len = length() + 1;
	 char* ch = new char[len]; //Added on 5th April - Vasu
	 strncpy_s(ch,len, GetCStr(), len); //Added on 5th April - Vasu
     //return strncpy(new char[len], GetCStr(), len); //Commented on 5th April - Vasu
	 return ch; //Added on 5th April - Vasu
    }
